...
esl_counts AS (
  SELECT SUM((CASE WHEN first_language != 'English' THEN 1 ELSE 0 END)) AS "ESL_COUNTS"
  FROM student_list
),
...
lbote_counts AS (
  SELECT COUNT(student_id) AS "LBOTE_COUNTS"
  
  FROM lbote_students
)

SELECT
  (SELECT * FROM esl_counts) AS "ESL_TOTAL",
  (SELECT * FROM lbote_counts) AS "LBOTE_TOTAL"
