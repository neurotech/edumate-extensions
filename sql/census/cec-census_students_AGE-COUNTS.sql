...
student_list AS (
  SELECT
    ...
    contact.surname,
    FLOOR((DATE(TO_CHAR((current date), 'YYYY') || '-07-01') - contact.birthdate) / 10000) AS "AGE",
    -- Count data
    contact.gender_id,
...
SELECT
  ...
  age_counts.age,
  age_counts.age_counts AS "TOTAL"

FROM age_counts

INNER JOIN form ON form.form_id = age_counts.form_id
INNER JOIN gender ON gender.gender_id = age_counts.gender_id
INNER JOIN indigenous ON indigenous.indigenous_id = age_counts.indigenous_id

ORDER BY form.form_id, age, gender DESC