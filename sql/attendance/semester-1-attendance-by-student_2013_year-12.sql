-- Year 12 Attendance Stats
-- 10th October 2012 to 13th May 2013
...
WITH STUDENT_ATTENDANCE_DATA AS (
  ...
  LEFT JOIN DAILY_ATTENDANCE_STATUS ATTENDANCE_STATUS_DAILY ON ATTENDANCE_STATUS_DAILY.DAILY_ATTENDANCE_STATUS_ID = DA.DAILY_ATTENDANCE_STATUS_ID
    ...
    AND DA.DATE_ON BETWEEN '2012-10-10' AND '2013-05-13'
  ...
  LEFT JOIN DAILY_ATTENDANCE_STATUS ATTENDANCE_STATUS_AM ON ATTENDANCE_STATUS_AM.DAILY_ATTENDANCE_STATUS_ID = DA.AM_ATTENDANCE_STATUS_ID
    AND ATTENDANCE_STATUS_AM.DAILY_ATTENDANCE_STATUS LIKE '%Late%'
  	...
  	AND DA.DATE_ON BETWEEN '2013-01-31' AND '2013-05-13'
  
  WHERE FORM_RUN = '2013 Year 12' AND (ATTENDANCE_STATUS_DAILY.DAILY_ATTENDANCE_STATUS IS NOT NULL OR ATTENDANCE_STATUS_AM.DAILY_ATTENDANCE_STATUS IS NOT NULL)
...
SELECT DISTINCT
  ...
  (CASE WHEN ALC.ROWNUMBER = 1 THEN '10th October 2012' ELSE NULL END) AS "REPORT_PERIOD_START",
  (CASE WHEN ALC.ROWNUMBER = 1 THEN '13th May 2013' ELSE NULL END) AS "REPORT_PERIOD_END"
