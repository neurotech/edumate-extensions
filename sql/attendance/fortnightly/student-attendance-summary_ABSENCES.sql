...
ABSENCES_LATES_COUNTS AS (
  SELECT
    ...
    SUM(CASE WHEN SAD.DATE_ON BETWEEN SAD.REPORT_FN_START AND SAD.REPORT_END AND (SAD.AM IN (3,4,5,6) AND SAD.PM IN (3,4,5,6)) THEN 1 ELSE 0 END) AS "EXPLAINED_ABSENCES",
    SUM(CASE WHEN SAD.DATE_ON BETWEEN SAD.REPORT_FN_START AND SAD.REPORT_END AND (SAD.AM IN (2,7) AND SAD.PM IN (2,7)) THEN 1 ELSE 0 END) AS "UNEXPLAINED_ABSENCES",
    SUM(CASE WHEN SAD.AM IN (2,3,4,5,6,7) AND SAD.PM IN (2,3,4,5,6,7) THEN 1 ELSE 0 END) AS "ABSENCES_YTD",
    SUM(CASE WHEN SAD.AM IN (3,4,5,6) AND SAD.PM IN (3,4,5,6) THEN 1 ELSE 0 END) AS "EXPLAINED_ABSENCES_YTD",
    SUM(CASE WHEN SAD.AM IN (2,7) AND SAD.PM IN (2,7) THEN 1 ELSE 0 END) AS "UNEXPLAINED_ABSENCES_YTD"
...
SELECT
  ...
  (CASE WHEN ROW_NUMBER() OVER () = 1 THEN ALC.DIFF ELSE NULL END) AS "FORTNIGHTS_YTD",
  ALC.FORTNIGHT_ABSENCES,
  ALC.EXPLAINED_ABSENCES,
  ALC.UNEXPLAINED_ABSENCES,
  ALC.ABSENCES_YTD,
  ALC.EXPLAINED_ABSENCES_YTD,
  ALC.UNEXPLAINED_ABSENCES_YTD,
...
ORDER BY FORM_RUN.FORM_RUN, ALC.ABSENCES_YTD DESC, CLASS.CLASS, CONTACT.SURNAME