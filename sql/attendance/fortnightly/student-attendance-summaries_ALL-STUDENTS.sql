...
STUDENT_ATTENDANCE_DATA AS (
  SELECT DISTINCT
    ...
    CLASS.CLASS,
    DA.DATE_ON,
    DA.DAILY_ATTENDANCE_STATUS_ID AS "ALLDAY",
...
ABSENCES_LATES_COUNTS AS (
  SELECT
    ...
    SUM(CASE WHEN SAD.ALLDAY IN (4,5,6) THEN 1 ELSE 0 END) AS "EXPLAINED_ABSENCES",
    SUM(CASE WHEN SAD.ALLDAY IN (2,7) THEN 1 ELSE 0 END) AS "UNEXPLAINED_ABSENCES",
    SUM(CASE WHEN SAD.ALLDAY IN (2,4,5,6,7) THEN 1 ELSE 0 END) AS "ABSENCES_YTD",
...
--WHERE exp_absences_ytd + unexp_absences_ytd != absences_ytd
