...
ABSENCES_LATES_COUNTS AS (
  SELECT
    ...
    SAD.STUDENT_ID,
    SUM(CASE WHEN SAD.DATE_ON BETWEEN SAD.REPORT_FN_START AND SAD.REPORT_END AND SAD.ALL_ABSENCES IS NOT NULL THEN 1 ELSE 0 END) AS "FORTNIGHT_ABSENCES",
    SUM(CASE WHEN SAD.ALL_ABSENCES IN (3,4,5,6) THEN 1 ELSE 0 END) AS "EXPLAINED_ABSENCES",
    SUM(CASE WHEN SAD.ALL_ABSENCES IN (2,7) THEN 1 ELSE 0 END) AS "UNEXPLAINED_ABSENCES",
    COUNT(SAD.ALL_ABSENCES) AS "ABSENCES_YTD",

    SUM(CASE WHEN SAD.DATE_ON BETWEEN SAD.REPORT_FN_START AND SAD.REPORT_END AND (SAD.AM IN (14,15,16,17,18,19) AND SAD.PM IN (14,15,16,17,18,19)) THEN 1 ELSE 0 END) AS "FORTNIGHT_LATES",
    SUM(CASE WHEN SAD.AM IN (16,17,18,19) AND SAD.PM IN (16,17,18,19) THEN 1 ELSE 0 END) AS "EXPLAINED_LATES",
    SUM(CASE WHEN SAD.AM IN (14,15) AND SAD.PM IN (14,15) THEN 1 ELSE 0 END) AS "UNEXPLAINED_LATES",
    SUM(CASE WHEN SAD.AM IN (14,15,16,17,18,19) AND SAD.PM IN (14,15,16,17,18,19) THEN 1 ELSE 0 END) AS "LATES_YTD"
...
ORDER BY SAD.FORM_RUN, ALC.ABSENCES_YTD DESC, SAD.SURNAME