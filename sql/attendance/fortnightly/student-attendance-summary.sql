...
STUDENT_ATTENDANCE_DATA AS (
  SELECT
    ...
    REPORT_VARS.HOLIDAY_WEEKS,
    (SELECT
      (SUM(CASE WHEN GTRD.TERM_DATE <= (CURRENT DATE) THEN 1 ELSE 0 END) / 10)
    FROM TABLE(EDUMATE.GET_TIMETABLE_RUNNING_DATES((SELECT TIMETABLE_ID FROM TERM WHERE TERM = 'Term 1' AND START_DATE LIKE (TO_CHAR((CURRENT DATE), 'YYYY')) || '-%%-%%' FETCH FIRST 1 ROW ONLY))) GTRD
...
SELECT DISTINCT
  ...
  ALC.ABSENCES_YTD,
  ALC.LATES_YTD,
  CAST((ALC.ABSENCES_YTD / SAD.DIFF) AS DECIMAL(5,2)) AS "CUMULATIVE_ABSENCES_AVG",
  (CASE WHEN ROWNUMBER = 1 THEN SAD.DIFF ELSE NULL END) AS "FORTNIGHT_FREQUENCY"
