...
SELECT
    TO_CHAR((SELECT report_start FROM report_vars),'DD Month YYYY') || ' to ' || TO_CHAR((SELECT report_end FROM report_vars),'DD Month YYYY')AS "REPORT_DATES",
    label ,
    teacher_name,
    periods,
    absent,
    ...
    attended
FROM final_report
ORDER BY final_report.teacher_name
