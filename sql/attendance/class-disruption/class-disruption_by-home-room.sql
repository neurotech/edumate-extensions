--[[From=date]]
--[[To=date]]
WITH date_range(date_on) AS
    (
    SELECT
        DATE('[[From=date]]') AS DATE_ON
    ...
    WHERE DATE(event.start_date) <= DATE('[[To=date]]') AND DATE(event.end_date) >= DATE('[[From=date]]')
    ...
    WHERE DATE(activity.start_date) <= DATE('[[To=date]]') AND DATE(activity.end_date) >= DATE('[[From=date]]')
    ),
...
/*        CASE WHEN students_on_event.student_id is not null THEN 'Student Event'
    ...
    GROUP BY student_homeroom.homeroom
    ),
    ...
    teacher_overall_stats AS
    (
    SELECT
        raw_data.contact_id,
        CAST(FLOAT(SUM(raw_data.to_attend))*100/COUNT(raw_data.to_attend) AS DECIMAL(4,1)) AS TO_ATTEND
    FROM raw_data
    GROUP BY raw_data.contact_id
    ),

    homeroom_students AS
    (
    SELECT 
        COALESCE(student_homeroom.homeroom,' No Home Room') AS HOMEROOM,
        ROW_NUMBER() OVER (PARTITION BY student_homeroom.homeroom ORDER BY contact.surname, contact.firstname) AS SORT_ORDER,
        student_form.form AS YR,
        UPPER(contact.surname)||', '||COALESCE(contact.preferred_name,contact.firstname) AS STUDENT,
        TO_CHAR(periods,'999') AS PERIODS,
        TO_CHAR(absent,'999') AS ABSENT,
        TO_CHAR(on_event,'999') AS ON_EVENT,
        TO_CHAR(appointment,'999') AS APPOINTMENT,
        TO_CHAR(staff_event,'999') AS STAFF_EVENT,
        TO_CHAR(staff_away,'999') AS STAFF_AWAY,
        TO_CHAR(staff_personal,'999') AS STAFF_PERSONAL,
        TO_CHAR(to_attend-absent,'999') AS ATTENDED       
    FROM student_overall_stats stats
        INNER JOIN student ON student.student_id = stats.student_id
    ...
    (
    SELECT
        COALESCE(homeroom,' No Home Room') AS HOMEROOM,
        10000 AS SORT_ORDER,
        '- -' AS YR,
        '- - TOTAL as % - -' AS STUDENT,
    ...
    homeroom_report AS
    (
    SELECT * FROM homeroom_totals
    UNION
    SELECT * FROM homeroom_students
    )

SELECT
    homeroom, homeroom||REPLACE(REPLACE(REPLACE(' ('||TO_CHAR(DATE('[[From=date]]'),' DD Mon')||' - '||TO_CHAR(DATE('[[To=date]]'),' DD Mon')||')',' 0',' '),'  ',' '),'( ','(') AS LABEL, yr, student, periods, absent, on_event, appointment, staff_event, staff_away, staff_personal, attended
FROM homeroom_report
ORDER BY homeroom_report.homeroom, homeroom_report.sort_order
