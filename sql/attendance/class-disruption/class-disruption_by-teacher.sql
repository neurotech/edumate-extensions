...
/*        CASE WHEN students_on_event.student_id is not null THEN 'Student Event'
    ...
    (
    SELECT
        COALESCE(s2.salutation||' ','')||c2.firstname||' '||c2.surname AS LABEL,
        c2.surname||', '||c2.firstname AS TEACHER_ORDER,
        ROW_NUMBER() OVER (PARTITION BY student_teacher_stats.contact_id ORDER BY contact.surname, contact.firstname) AS SORT_ORDER,
    ...
    FROM student_teacher_stats 
        INNER JOIN contact c2 ON c2.contact_id = student_teacher_stats.contact_id
        LEFT JOIN salutation s2 ON s2.salutation_id = c2.salutation_id
        INNER JOIN student ON student.student_id = student_teacher_stats.student_id
    ...
    UNION
    SELECT * FROM teacher_totals

    )

SELECT
    label||REPLACE(REPLACE(REPLACE(' ('||TO_CHAR(DATE('[[From=date]]'),' DD Mon')||' - '||TO_CHAR(DATE('[[To=date]]'),' DD Mon')||')',' 0',' '),'  ',' '),'( ','(') AS LABEL, student_name, homeroom, periods, absent, on_event, appointment, staff_event, staff_away, staff_personal, attended
FROM final_report
