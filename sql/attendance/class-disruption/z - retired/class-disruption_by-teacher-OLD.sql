...
SELECT
    label||REPLACE(REPLACE(REPLACE(' ('||TO_CHAR((SELECT report_start FROM report_vars),' DD Mon')||' - '||TO_CHAR((SELECT report_end FROM report_vars),' DD Mon')||')',' 0',' '),'  ',' '),'( ','(') AS LABEL, student_name, homeroom, periods, absent, on_event, appointment, staff_event, staff_away, staff_personal, attended
FROM final_report
