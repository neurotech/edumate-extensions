...
raw_data AS (
  ...
  INNER JOIN class ON class.class_id = period_class.class_id
    AND class.class_type_id NOT IN (2,6,4,8,1000,1001,1002,1003)
...
class_students AS (
  SELECT
    ...
    TO_CHAR(student_class_stats.periods,'9990.0') AS PERIODS,
  ...
  LEFT JOIN scheduled_periods ON scheduled_periods.student_id = student_class_stats.student_id AND scheduled_periods.class_id = student_class_stats.class_id
)
