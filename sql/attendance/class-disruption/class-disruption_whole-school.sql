...
SELECT
    form_run, 'Whole School'||REPLACE(REPLACE(REPLACE(' ('||TO_CHAR(DATE('[[From=date]]'),' DD Mon')||' - '||TO_CHAR(DATE('[[To=date]]'),' DD Mon')||')',' 0',' '),'  ',' '),'( ','(') AS LABEL, periods, absent, on_event, appointment, staff_event, staff_away, staff_personal, attended, students
