WITH report_vars AS (
  SELECT
/*     ('rmattoni') AS "USERNAME",
    DATE('2015-07-13') AS "REPORT_START",
    DATE('2015-08-06') AS "REPORT_END" */
...
student_classes AS (
  ...
  WHERE
    student_id = (SELECT student_id FROM student WHERE contact_id = (SELECT contact_id FROM sys_user WHERE LOWER(username) = (SELECT username FROM report_vars)))
    AND
...
teacher_timetable AS (
  ...
  -- Join all students on all periods
  LEFT JOIN student_classes ON student_classes.class_id = period_class.class_id
  ...
  WHERE
    student_classes.student_id IS NOT null
    AND
