...
SELECT
  ...
  CASE WHEN
   TO_DATE > (REPORT_VARS.REPORT_END)
...
WHERE
  AWAY_REASON_ID IN (1,3,5,8,74,75,97,98,121,145,146,147,148)
    AND
...
(
  ...
  WHERE
    SA.AWAY_REASON_ID IN (1,3,5,8,74,75,97,98,121,145,146,147,148)
      AND
...
SELECT
  ...
  (CASE WHEN SAD.DAYS_ABSENT < 1.00 THEN 'Partial' ELSE CAST(SAD.DAYS_ABSENT AS VARCHAR(5)) END) AS "DAYS_ABSENT_PAY_PERIOD",
  (CASE WHEN SAD.DAYS_ABSENT < 1.00 THEN CHAR(TIME(SAD.EFFECTIVE_START),USA) || ' - ' || CHAR(TIME(SAD.EFFECTIVE_END),USA) ELSE NULL END) AS "HOURS_ABSENT",
  YTD_SUM.DAYS_ABSENT_THIS_YEAR,
