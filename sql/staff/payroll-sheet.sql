...
WHERE
  AWAY_REASON_ID IN (1,3,5,8,74,75,97,98,121,145,146,147,148,169)
    AND
...
SELECT
  ...
  (CASE WHEN SAD.DAYS_ABSENT < 1.00 THEN CHAR(TIME(SAD.EFFECTIVE_START),USA) || ' - ' || CHAR(TIME(SAD.EFFECTIVE_END),USA) ELSE '-' END) AS "HOURS_ABSENT",
