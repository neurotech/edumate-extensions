WITH aways AS (
  SELECT staff.contact_id
  FROM staff_away
  INNER JOIN staff ON staff.staff_id = staff_away.staff_id
  WHERE DATE(from_date) = (current date - 3 DAYS) AND away_reason_id = 1
),
...
student_courses AS (
  ...
  INNER JOIN report_period_form_run ON report_period_form_run.report_period_id = report_period.report_period_id 
    ...
    AND report_period.report_period LIKE '2015 Semester 1 Year %'  
...
-- join students
  ...
  WHERE course.course not like '%Core%' AND course.course not like '%Administration%' 
    ...
    AND c2.contact_id IN (SELECT contact_id FROM aways)
),
...
student_course_outcomes AS (
  SELECT
    ...
    DATE(stud_ind_mark.last_updated) AS LAST_UPDATED
  ...
  GROUP BY student_courses.report_period_id, student_courses.report_period, student_courses.department, student_courses.student_id, student_courses.course_id, student_courses.class, student_courses.teacher, DATE(stud_ind_mark.last_updated)
)
...
WHERE sco.scored BETWEEN 1 AND sco.outcomes AND sco.last_updated = (current date - 3 DAYS)
