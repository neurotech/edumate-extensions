...
SELECT
  ROWNUMBER() OVER (PARTITION BY SA.STAFF_ID) AS "SORT_ORDER",
  SA.STAFF_ID,
  SA.AWAY_REASON_ID,
  SA.FROM_DATE,
  SA.TO_DATE,
...
YTD_SUM AS (
  SELECT
    ...
    (SELECT * FROM TABLE(DB2INST1.BUSINESS_DAYS_COUNT(
...
SELECT
  ...
  SAD.EFFECTIVE_END,
  SAD.DAYS_ABSENT,
  CASE WHEN SAD.SORT_ORDER > 1 THEN NULL ELSE YTD_SUM.DAYS_ABSENT_THIS_YEAR END AS "WHOLE_DAYS_ABSENT_THIS_YEAR",
  CASE WHEN SAD.SORT_ORDER > 1 THEN NULL ELSE YTD_SUM.BUSINESS_DAYS_TD END AS "BUSINESS_DAYS_TD"
