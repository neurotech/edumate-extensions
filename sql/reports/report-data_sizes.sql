WITH report_student_courses as (
  ...
  WHERE report_period.report_period_id = 291
--  WHERE report_period.report_period_id = '[[Report Period:query_list(select report_period_id from report_period where academic_year_id = (select academic_year_id from academic_year where academic_year = YEAR(CURRENT DATE)) and completed is null ORDER BY semester_id desc, start_date desc)]]'
),
...
outlines as (
  SELECT class.course_id, co.class_id, co.course_outline
