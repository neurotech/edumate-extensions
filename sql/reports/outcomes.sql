WITH active_subjects AS
    ...
    WHERE view_student_class_enrolment.start_date <= DATE('[[As at=date]]')
        AND view_student_class_enrolment.end_date >= DATE('[[As at=date]]')
    ),
    ...
    FROM course_indicator
        ...
        INNER JOIN timetable ON timetable.academic_year_id = academic_year.academic_year_id
            AND timetable.computed_v_start_date <= DATE('[[As at=date]]')
            AND timetable.computed_end_date >= DATE('[[As at=date]]')
...
SELECT
    ...
    CASE WHEN ci1.indicator_id is null OR ci1.indicator_id = 0 THEN '' ELSE 'Y' END AS "SEMESTER1",
    CASE WHEN ci2.indicator_id is null OR ci2.indicator_id = 0 THEN '' ELSE 'Y' END AS "SEMESTER2"
