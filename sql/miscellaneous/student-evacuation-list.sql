WITH RAW_REPORT AS (
  ...
  LEFT JOIN DAILY_ATTENDANCE DA ON DA.STUDENT_ID = GCES.STUDENT_ID AND DA.DATE_ON = (CURRENT DATE)
  LEFT JOIN DAILY_ATTENDANCE_STATUS DAS ON DAS.DAILY_ATTENDANCE_STATUS_ID = DA.AM_ATTENDANCE_STATUS_ID
  INNER JOIN VIEW_STUDENT_CLASS_ENROLMENT VSCE ON VSCE.STUDENT_ID = GCES.STUDENT_ID AND
    (
      VSCE.CLASS_TYPE_ID = 2 AND
      VSCE.ACADEMIC_YEAR = TO_CHAR((CURRENT DATE), 'YYYY') AND
      VSCE.END_DATE > (CURRENT DATE)
    )
  LEFT JOIN CLASS ON CLASS.CLASS_ID = VSCE.CLASS_ID

  LEFT JOIN TABLE(EDUMATE.GET_STUDENT_LOCATION(GCES.STUDENT_ID, (CURRENT TIMESTAMP))) GSL ON GSL.STUDENT_ID = GCES.STUDENT_ID
...
ORDER BY HOMEROOM, SURNAME, FIRSTNAME