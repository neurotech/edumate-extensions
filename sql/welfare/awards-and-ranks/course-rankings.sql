WITH selected_period AS
    ...
    FROM report_period
    --WHERE report_period.report_period = '[[Report Period=query_list(SELECT report_period FROM report_period WHERE LOWER(report_period) LIKE '%ranking%' AND end_date >= current_date - 2 YEARS AND start_date <= current_date ORDER BY report_period)]]'
    WHERE report_period.report_period = '2014 Year 12 Ranking'
    ),
    ...
    SELECT
        ...
        /*SUM(CASE WHEN MONTH(report_period.start_date) IN (1,2) 
                THEN (CASE WHEN MONTH(COALESCE(coursework_extension.due_date,coursework_task.due_date)) IN (1,2,3,4,5,6) THEN 1 ELSE 0 END)
                ELSE (CASE WHEN MONTH(COALESCE(coursework_extension.due_date,coursework_task.due_date)) IN (10,11,12,1,2,3) THEN 1 ELSE 0 END) END)
            AS S1_TASKS,*/
    ...
    SELECT
        ...
        --CASE WHEN ordered_task_results.ranked_units <= 10 THEN ordered_task_results.units WHEN ordered_task_results.ranked_units = 11 AND ordered_task_results.units = 2 THEN 1 ELSE 0 END AS UNITS,
        --ordered_task_results.weight,
        course_rank AS RANK,
        --CASE WHEN course_rank = 1 THEN '**' WHEN course_rank <= (FLOAT(students) / 10 + 0.5) THEN '*' ELSE '' END AS AW,
        (CASE
