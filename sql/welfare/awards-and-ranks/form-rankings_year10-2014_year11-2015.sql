WITH report_vars AS (
  SELECT
    '2014 Year 10 Ranking' AS PREVIOUS,
    '2015 Year 11 Ranking' AS CURRENT
...
previous_course_units(unit_number) AS
    ...
    previous_raw_course_results AS
    (
    SELECT
    ...
    WHERE task.mark_out_of > 0 AND task.weighting > 0 AND stud_task_raw_mark.student_id IN (SELECT student_id FROM previous_student_form)
    ...
    FROM previous_raw_course_results
    ...
    previous_final_scores AS
    (
    SELECT
        report_period_id,
        previous_course_rankings.student_id,
        (CASE
          -- Random electives
        ...
        previous_course_rankings.course_id,
        course,
        units,
        total_units,
        final_mark,
        ...
        (CASE
          ...
          ELSE ROUND(((final_mark - previous_sdmean.mean) / previous_sdmean.sd * 12.5 + 60),3)
        END) AS "FINAL_SCALED_MARK",
        RANK() OVER (PARTITION BY previous_course_rankings.course_id ORDER BY ROUND((CASE WHEN cohort < 15 THEN final_mark ELSE ROUND(((final_mark - previous_sdmean.mean) / previous_sdmean.sd * 12.5 + 60),3) END),0) DESC) AS COURSE_RANK
        
        -- Old:
        --ROUND(CASE WHEN max_mark = min_mark THEN 100 ELSE(final_mark - min_mark) *100 / (max_mark - min_mark) END,3) AS FINAL_SCORE,
        --course_rank
        
    FROM previous_course_rankings
    
    INNER JOIN previous_included_students ON previous_included_students.student_id = previous_course_rankings.student_id
    INNER JOIN previous_sdmean ON previous_sdmean.course_id = previous_course_rankings.course_id
    
    WHERE qualifying_units >= 16
    ),

    previous_unit_results AS
    (
    SELECT
        report_period_id,
        student_id,
    ...
    FROM previous_final_scores
    INNER JOIN previous_course_units ON previous_course_units.unit_number <= previous_final_scores.units
    ),

    previous_ranked_unit_results AS
    (
    SELECT
        report_period_id,
        student_id,
    ...
    FROM previous_unit_results
    ),

    previous_best_x_units AS
    (
    SELECT
        report_period_id,
        student_id,
    ...
    FROM previous_best_x_units
    
    INNER JOIN course ON course.course_id = previous_best_x_units.course_id
    
    LEFT JOIN course_final_mark ON course_final_mark.course_id = course.course_id
      AND course_final_mark.report_period_id = previous_best_x_units.report_period_id
    
    GROUP BY student_id, total_units
    ),

    previous_raw_report AS
    (
    SELECT
        previous_best_courses.student_id,
        ...
        RANK() OVER (ORDER BY best_weighted_rank ASC) AS RANK,
        (CASE
    ...
    FROM previous_best_courses

    INNER JOIN student ON student.student_id = previous_best_courses.student_id
    INNER JOIN contact ON contact.contact_id = student.contact_id
    LEFT JOIN previous_student_form ON previous_student_form.student_id = student.student_id
    ),
        
    previous_rank_final AS (
      SELECT student_id, rank FROM previous_raw_report ORDER BY RANK
    ),

    course_units(unit_number) AS
    (
    SELECT
        INTEGER(1)
    FROM sysibm.sysdummy1
    ...
    SELECT report_period_id
    FROM report_period
    WHERE report_period.report_period = (SELECT current FROM report_vars)
    ),
    ...
    SELECT
        ...
        SUM(CASE WHEN course_rank = 3 THEN 1 ELSE 0 END) AS THIRDS,
        total_units,
        LISTAGG('[' || course_rank || CASE WHEN best_units = 0 THEN '*' ELSE '' END || '] ' || '(' || REPLACE(TO_CHAR(FLOAT(final_scaled_mark), '990.00'), '  ', '') || ') ' || COALESCE(course.print_name,course.course),', ') WITHIN GROUP(ORDER BY core_or_elective DESC, best_units DESC, course_rank) AS RANKING_DETAILS,
        SUM(FLOAT(course_rank)*COALESCE(course_final_mark.weight,1)*units) / SUM(COALESCE(course_final_mark.weight,1)*units) AS OVERALL_WEIGHTED_RANK,
        SUM(FLOAT(final_mark)*COALESCE(course_final_mark.weight,1)*units) / SUM(COALESCE(course_final_mark.weight,1)*units) AS OVERALL_WEIGHTED_MARK,
        SUM(FLOAT(final_scaled_mark)*COALESCE(course_final_mark.weight,1)*units) / SUM(COALESCE(course_final_mark.weight,1)*units) AS OVERALL_WEIGHTED_SCALED_MARK,
        SUM(FLOAT(best_rank)*COALESCE(course_final_mark.weight,1)*best_units) / SUM(COALESCE(course_final_mark.weight,1)*best_units) AS BEST_WEIGHTED_RANK,
    ...
    SELECT
        ...
        previous_rank_final.rank AS PREVIOUS_YEAR_RANK,
        (CASE
          WHEN RANK() OVER (ORDER BY best_weighted_rank ASC) = 1 THEN '**'
          WHEN RANK() OVER (ORDER BY best_weighted_rank ASC) <= FLOAT(COUNT(student.student_id) OVER (PARTITION BY 1)) * 15 / 100 + 0.5 AND RANK() OVER (ORDER BY best_weighted_rank ASC) <= 10 THEN '*'
          ELSE ''
    ...
    INNER JOIN student_homeroom ON student_homeroom.student_id = best_courses.student_id
    
    LEFT JOIN student_form ON student_form.student_id = student.student_id
    LEFT JOIN previous_rank_final ON previous_rank_final.student_id = best_courses.student_id
    )

SELECT
  (SELECT current FROM report_vars) AS "REPORT_PERIOD",
  printed_at,
  (CASE WHEN raw_report.previous_year_rank IS null THEN '--' ELSE CHAR(raw_report.previous_year_rank) END) AS "PREVIOUS_YEAR_RANK",
  (CASE
    WHEN CHAR((CASE WHEN raw_report.previous_year_rank IS NUll THEN pos ELSE raw_report.previous_year_rank END) - pos) > 0 THEN '+' || CHAR((CASE WHEN raw_report.previous_year_rank IS NUll THEN pos ELSE raw_report.previous_year_rank END) - pos)
    ELSE CHAR((CASE WHEN raw_report.previous_year_rank IS NUll THEN pos ELSE raw_report.previous_year_rank END) - pos)    
  END) AS "DIFF",
  pos,
  aw,
  student,
  form,
  REPLACE(REPLACE(homeroom, ' Home Room ', ''), RIGHT(REPLACE(homeroom, ' Home Room ', ''), 3), '') AS HOUSE,
...
--ORDER BY ((CASE WHEN raw_report.previous_year_rank IS NULL THEN pos ELSE raw_report.previous_year_rank END) - pos) DESC, surname, firstname
ORDER BY REPLACE(REPLACE(homeroom, ' Home Room ', ''), RIGHT(REPLACE(homeroom, ' Home Room ', ''), 3), ''), ((CASE WHEN raw_report.previous_year_rank IS NUll THEN pos ELSE raw_report.previous_year_rank END) - pos) DESC