WITH course_units(unit_number) AS
    ...
    SELECT
        report_period_id
    FROM report_period
    WHERE --report_period.report_period = '2014 Year 07 Ranking'
    ...
    SELECT
        ...
        (CASE WHEN course_rankings.department_id = 27 THEN 'E' ELSE 'C' END) AS "CORE_OR_ELECTIVE",
    ...
    SELECT
        ...
        course_rank,
        (CASE
          WHEN core_or_elective = 'C' THEN MAX(CASE WHEN core_or_elective = 'C' AND ranked_units <= 20 THEN final_mark ELSE null END)
          WHEN core_or_elective = 'E' THEN MAX(CASE WHEN core_or_elective = 'E' AND ranked_units <= 2 THEN final_mark ELSE null END)
        END) AS "BEST_MARK",
        (CASE
          WHEN core_or_elective = 'C' THEN MAX(CASE WHEN core_or_elective = 'C' AND ranked_units <= 20 THEN final_scaled_mark ELSE null END)
        ...
        (CASE
          WHEN core_or_elective = 'C' THEN SUM(CASE WHEN core_or_elective = 'C' AND ranked_units <= 20 THEN 1 ELSE 0 END)
          WHEN core_or_elective = 'E' THEN SUM(CASE WHEN core_or_elective = 'E' AND ranked_units <= 2 THEN 1 ELSE 0 END)
        END) AS "BEST_UNITS",       
        
        (CASE
          WHEN core_or_elective = 'C' THEN MAX(CASE WHEN core_or_elective = 'C' AND ranked_units <= 20 THEN course_rank ELSE null END)
