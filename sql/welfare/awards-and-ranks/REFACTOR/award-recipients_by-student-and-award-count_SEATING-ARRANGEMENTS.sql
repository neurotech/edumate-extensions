...
aggregated_awards AS (
  SELECT
    student_id,
    LISTAGG(award, ', ') WITHIN GROUP(ORDER BY student_id) AS "AWARDS",
    COUNT(award) AS "TOTAL"
...
SELECT DISTINCT
  ...
  aggregated_awards.awards,
  aggregated_awards.total
...
-- Aggregated Awards
INNER JOIN aggregated_awards ON aggregated_awards.student_id = award_winners.student_id

ORDER BY aggregated_awards.total DESC, surname, firstname