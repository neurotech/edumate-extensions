...
unsorted AS (
  ...
  INNER JOIN view_student_form_run vsfr ON vsfr.student_id = award_winners.student_id AND vsfr.academic_year = (SELECT YEAR(report_end) FROM report_vars) AND vsfr.start_date <= (SELECT report_start FROM report_vars) AND vsfr.end_date >= (SELECT report_end FROM report_vars)
  
  -- Homeroom
  INNER JOIN view_student_class_enrolment vsce ON vsce.student_id = award_winners.student_id AND vsce.class_type_id = 2 AND vsce.academic_year = (SELECT YEAR(report_end) FROM report_vars) AND vsce.start_date <= (SELECT report_start FROM report_vars) AND vsce.end_date >= (SELECT report_end FROM report_vars)
